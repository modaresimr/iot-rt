...
class Formr
{
    ...
    protected function _check_filesize($handle,$label='')
    {
        ...
        if ($handle['size'] > $this->upload_max_filesize) {
            ...
            $this->errors[$handle['key']] = '<strong>'.$label.'</strong> File size exceeded. The file can not be larger than ' . $size;
    ...
    protected function _mycheck_upload_accepted_types($handle,$lable='',$rules='')
    {
        if(empty($rules))
            $rules='pdf|mp4|zip|tex|jpg|png|csv|doc|docx|ppt|pptx';

        $acceptedTypes=explode('|',$rules); 
        # see if it's in the accepted upload types
        if (!in_array($handle['ext'], $acceptedTypes)) {
            //$this->errors['accepted-types'] = 'Oops! The file was not uploaded because it is in an unsupported file type. Accepted formats are:'.$rules;
            $this->errors[$handle['key']] = '<strong>'.$label.'</strong> The file was not uploaded because it is in an unsupported file type. Accepted formats are:'.$rules;
            return true;
        }
        return false;
    }
    ...
    protected function _upload_files($name,$label='',$rules='')
    {
        if(empty($label))
            $label=$name;
        ...
        if (!empty($_FILES[$name]['tmp_name'])) {
            ...
            if (isset($_FILES[$name]['error']['0'])) {
                ...
                for ($i = 0; $i < count($_FILES[$name]['tmp_name']); $i++) {
                    switch($_FILES[$name]['error'][$i]){
                        case UPLOAD_ERR_NO_FILE:
                            if($this->_check_required($name)){
                                $form->errors[$name]='<strong>'.$label.'</strong> is required';    
                            }
                            break;
                        case UPLOAD_ERR_INI_SIZE:
                        case UPLOAD_ERR_FORM_SIZE:
                            $form->errors[$name]='<strong>'.$label.'</strong> exceeded the max size.';
                            break;
                        default:
                            $form->errors[$name]='<strong>'.$label.'</strong> has an expected error';
                    }
                    $handle['error']=$_FILES[$name]['error'][$i];
                    if (!empty($_FILES[$name]['tmp_name']) && is_uploaded_file($_FILES[$name]['tmp_name'][$i])) {
                        ...
                        if($this->uploads){
                            ...
                            if($this->_mycheck_upload_accepted_types($handle,$label,$rules))
                                $handle['error']=-1;
                            if($this->_check_filesize($handle,$label))
                                $handle['error']=-2;
                ...
                $handle['error']=$_FILES[$name]['error'];
    ...
    protected function _post($name, $label = '', $rules = '')
    {
        ...
        if (!empty($_FILES[$name]['tmp_name'])) {
            ...
            if ($this->uploads && !$this->upload_dir) {
                $this->errors[$name] = 'Please specify an upload directory.';
                return false;
            }
            if ($this->uploads && !$this->upload_accepted_types && !$this->upload_accepted_mimes) {
                if (!empty($data['type']) == 'file') {
                    $this->errors[$name] = 'Please specify the type of file allowed for upload.';
                    return false;
                }
            }
            if ($return = $this->_upload_files($name,$label,$rules)) {
                return $return;
            }
        }
    ...
    # SELECT MENU
    protected function _my_create_select($data)
    {
        // if($this->method=="get"&&empty($_POST)){
        //     $_POST=$_GET;
        // }
        # echo an error if the field name hasn't been supplied
        if (!$this->is_not_empty($data['name'])) {
        ...
        $options_inline=$data['options_inline'];
        # options are user-defined
        # loop through the options array
        foreach ($data['options'] as $key => $value) {
            # if $value is an array, create an optgroup
            if (is_array($value)) {
                ...
                # if the form has been posted, print selected option
                $option_inline= $options_inline[$key]??'';
                if (isset($_POST[$data['name']]) && $_POST[$data['name']] == $key) {
                    $return .= $this->_t(2) . '<option value="' . $key . '" '.$option_inline.' selected="selected">' . $value . '</option>' . $this->_nl(1);
                }
                # print selected option on form load
                elseif (!isset($_POST[$data['name']]) && $data['selected'] == $key || (is_array($data['selected']) && in_array($key, $data['selected']))) {
                    # populate the field's value (on page load) with the session value
                ...
                # print remaining options
                    ...
                    if (!isset($_POST[$data['name']]) && $data['value'] == $key) {
                        ...
                        $return .= $this->_t(2) . '<option value="' . $key . '" '.$option_inline.'>' . $value . '</option>' . $this->_nl(1);
                    }
                }
            }
        }
    ...
    public function input_myselect($data, $label = '', $value = '', $id = '', $string = '', $inline = '', $selected = '', $options = '',$options_inline='')
    {
        ...
        return $this->_my_create_select($data);
    }
    ...
    protected function _create_label($data)
    {
        ...
        if ($this->wrapper == 'bootstrap') {
            # bootstrap control-label
            $return .= ' class="' . $this->controls['label']. ' ' .($this->in_errors($data['name'])?$this->controls['text-error']:''). '"';
        }
